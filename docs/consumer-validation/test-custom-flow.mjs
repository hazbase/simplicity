...
const simfPath = path.join(projectDir, 'contract.simf');
const artifactPath = path.join(projectDir, 'custom.artifact.json');
...
async function main() {
  ...
  await writeFile(path.join(projectDir, 'custom-flow-result.json'), JSON.stringify(result, null, 2));
